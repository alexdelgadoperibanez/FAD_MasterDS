...
PC2= pca2[[2]][,2]
PC3= pca2[[2]][,3]
componentes_princ <- cbind(PC1,PC2,PC3)
componentes_princ
library(ade4)
```
...
```{r}
fviz_pca_ind(pca2,axes=c(1,3), col.ind = train$Gama, alpha.ind = "contrib" ) + theme_grey()
```

```{r}
fviz_pca_ind( pca2,axes=c(1,3),  habillage = as.factor( train$Gama ), addEllipses = TRUE, ellipse.level = 0.99 )
```

```{r}
fviz_pca_ind( pca2, axes=c(1,3),col.ind = train$Make, alpha.ind = "contrib" ) + theme_grey()
```
```{r}
fviz_pca_ind( pca2, axes=c(1,3),col.ind = train$Fuel_Type, alpha.ind = "contrib" ) + theme_grey()
```

```{r}
fviz_pca_ind( pca2, axes=c(1,3),habillage = as.factor( train$Fuel_Type ), addEllipses = TRUE, ellipse.level = 0.99 )
```


```{r}
fviz_pca_ind( pca2, axes=c(1,3),col.ind = train$Transmission, alpha.ind = "contrib" ) + theme_grey()
```

```{r}
fviz_pca_ind( pca2, axes=c(1,3),habillage = as.factor( train$Transmission ), addEllipses = TRUE, ellipse.level = 0.99 )
```

```{r}
fviz_pca_ind( pca2,axes=c(1,3), col.ind = train$Location, alpha.ind = "contrib" ) + theme_grey()
```

```{r}
fviz_pca_biplot( pca2, axes=c(1,3),col.ind = train$Gama )
```
...
```
A la luz de los datos visuales recabados parece evidente que nos tenemos que quedar con las 3 primeras componentes principales (PC1, PC2 y PC3), pero no está tan claro que debemos hacer con la cuarta(PC4).
...
s.corcircle(componentes_princ[,-2], sub="PC1 Y PC3")
```

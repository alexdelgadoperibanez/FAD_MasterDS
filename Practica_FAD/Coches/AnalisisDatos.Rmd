...
```{r Limpieza ColNames}
colnames(data) <- c("X", "Name", "Location", "Year", "Total_Km", "Fuel_Type", "Transmission", "Owner_Type",
                    "Mileage (km/l)", "Engine (CC)", "Power (bhp)", "Seats", "New_Price (Lakh)", "Price (Lakh)")

```

```{r Limpieza Mileage}
Mileage <- substr(Mileage, 1, nchar(Mileage)-5)
data
```
...
data %>% select(Name, Engine (CC), Seats, Power (bhp), New_Price (Lakh), Price (Lakh)) %>% filter(is.na(Engine (CC)) | is.na(Seats) | is.na(Power (bhp)) | is.na(New_Price (Lakh)) | is.na(Price (Lakh))) %>% arrange(Name) %>% distinct()

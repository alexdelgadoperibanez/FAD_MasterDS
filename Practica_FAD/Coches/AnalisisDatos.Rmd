...
```{r Modificación de datos, results = TRUE, warning=FALSE}
...
```{r missing data, message=FALSE, results='hide'}
...
```{r missing data2, message=FALSE, results='hide'}
md.pattern(train, plot = TRUE, rotate.names=TRUE)
```
...
ggplot(train, aes(train$Year, train$Price))+geom_boxplot(aes(group=Year))+geom_jitter(alpha=0.1)+geom_smooth(method="loess")+scale_y_log10()

...
summary(Kilometers_Driven) #6019 es el total
boxplot(Kilometers_Driven) 
summary(Kilometers_Driven)
hist(Kilometers_Driven)
boxplot(Kilometers_Driven, horizontal=TRUE)
k <- hist(Kilometers_Driven)
k
...
s <- hist(Seats)
s
data %>% filter(Seats>0) -> data #quito outlier seats=0
summary(data$Seats)
```
...
summary(New_Price)
```
muchos na's planteamos q hacer
...
una vez analizados tanto las variables de manera univariente como sus outliers, Ahora ya podemos separar el dataset en sets para train y test.
...
```{r}
train %>% pull(Mileage) %>% str_split(" ", simplify=TRUE) %>% cbind(Fuel_Type) %>% subset(select=2:3) %>% as.data.frame() %>% table() #Extraigo los diferentes tipos que hay y como se relacionan

train %>% pull(Engine) %>% str_split(" ", simplify=TRUE) %>% subset(select=2) %>% table() # hago lo mismo para confirmar que solo hay un tipo

train %>% pull(Power) %>% str_split(" ", simplify=TRUE) %>% subset(select=2) %>% table() # hago lo mismo para confirmar que solo hay un tipo

train %>% pull(New_Price) %>% str_split(" ", simplify=TRUE) %>% subset(select=2) %>% table()

train %>% mutate(Owner_Type = factor(Owner_Type, levels=c("First", "Second", "Third", "Fourth & Above"))) -> train
...
```{r}
data <- na.omit(train) #omito na's para la correlacion, de momento
var(train) # Matriz de varianzas y covarianzas
cor(train) # Matriz de correlaciones
```
...
library(corrplot)
library(Hmisc)
plotcorr(cor(train)) # Para ver la correlación mejor

...
```{r}
summary(bateriasB)
boxplot(bateriasB$lifetime, horizontal=TRUE, col="yellow", border = "blue",whiskcol = "blue", whisklty = 2,lty = 1, xlab="lifetime(horas)", main="Baterías tipo B")
...
Sí parece seguir una distribución normal. La gráfica sigue una distribución aparentemente centrada en la media pero no forma la campana de Gauss perfecta. Por ello, y para asegurarnos, vamos a realizar los test de normalidad.
...
```{r, warning=FALSE}
qqnorm(bateriasB$lifetime, pch = 20, col = alpha("yellow", 2))
grid()
...
Con el test de normalidad de Shapiro no podemos rechazar que se siga una distribución normal porque obtenemos un pvalor muy alto teniendo en cuenta que la hipótesis nula es que se sigue esta distribución normal con un valor de alpha de 0.05. 
...
pnorm(q=210, mean = 200.50866, sd=2.71818, lower.tail=FALSE) #lowertail false coge la parte de la derecha, es decir, el mayor que
```
...
#q es 0 porque estamos buscando la probabilidad de que haya 0 defectuosas
```
...
12*5
#12 defectuosas cada dia por los 5 dias q esta abierta la fabrica, en total sese ta piezas defectjksas a la semana
```
...
simulacionWeibull <- data.frame(rweibull(n =5000, shape = 100, scale = 185)) #shape y scale son los parámetros de la distribución Weibull, ie, a=100 y b=185

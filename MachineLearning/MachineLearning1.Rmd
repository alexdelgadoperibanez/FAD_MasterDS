...
q <- quantregForest(data[,c(2:10,13,14)], as.factor(data[,15]))
forest1
```
...
tree <- predict(q, newdata= xtest)
# xtest <- rbind(forest1[1, ] , dataTest)
# xtest <- xtest[-1,]
# y.pred.test.rf=predict(forest1, xtest[,c(2:10,14,15)], type='response')
# table.libreria.test=table(y.pred.test.rf, as.factor(dataTest[,13]))
# table.libreria.test=table(tree, as.factor(dataTest[,13]))
...
Aún que los resultados son parecidos en 3 de los 4 kernels, hay uno que sobresale y es el kernel "Radial" 

...
library(randomForest)
set.seed(1234567)
forest1 <- randomForest(data[,c(2:10,13,14)], as.factor(data[,15]), ntree=100, importance=FALSE, proximity=TRUE, mtry=4, replace=FALSE)
forest1
```

```{r}
dataTest[,c(2:10,14,15)]
forest1
predict(forest1, newdata=nuevo_df, type='response')

y.pred.test.rf=predict(forest1, dataTest[,c(2:10,14,15)], type='response')
table.libreria.test=table(y.pred.test.rf, as.factor(dataTest[,13]))
y.pred.train.rf= predict(forest1, data[,c(2:10,13,14)])
table.libreria.train=table(y.pred.train.rf, as.factor(data[,15]))

...
Comenzaremos viendo los algoritmos de aprendizaje no supervisado en primer lugar.
...
Hacemos una división de los coches en gama baja-media y gama alta para poder realizar la regresión logística sobre la nueva variable. 
...
Y en segundo lugar, terminaremos aplicando los algoritmos de aprendizaje supervisado.
Antes de comenzar con ello, lo que debemos hacer es definir una medida de precisión para contrastar los datos una vez que tengamos cada matriz de confusión y comparar los resultados que nos ofrecen cada uno de los métodos que empleemos. En nuestro caso, la variable respuesta no está muy bien balanceada:

```{r análisis supervisado}
summary(data$Gama)
```

Como vemos, los coches de gama alta se corresponden con el 30% aproximadamente y los de gama baja-media, el 70%. dos categorías están equilibradas. Además, no queremos dar más valor a identificar un tipo de coche frente al otro. Por tanto, elegiremos la siguiente **medida de precisión**.
...
Una vez que tenemos definida nuestra medida de precisión y sabemos que el algoritmo utiliza las distancias euclídeas, tenemos que preguntarnos cuál es el número de vecinos más cercanos que deberíamos escoger para comparar cada coche. Usamos la siguiente fórmula que nos da un número orientativo de vecinos:
...
dataTest %>% 
  mutate(
    Gama = case_when( 
      ...
      dataTest$Make=="Skoda"|dataTest$Make=="Renault" |dataTest$Make=="Ford" |dataTest$Make=="Honda"|dataTest$Make=="Volkswagen" |dataTest$Make=="Hyundai" |dataTest$Make=="Nissan" |dataTest$Make=="Maruti" ~ "Gama baja-media",
      dataTest$Make=="Bentley"|dataTest$Make=="Porsche" |dataTest$Make=="Land Rover" |dataTest$Make=="Jaguar" |dataTest$Make=="Mini" |dataTest$Make=="Mercedes-Benz" |dataTest$Make=="Audi" |dataTest$Make=="Bmw" |dataTest$Make=="Jeep" |dataTest$Make=="Volvo" |dataTest$Make=="Isuzu" |dataTest$Make=="Mitsubishi" |dataTest$Make=="Toyota" |dataTest$Make=="Force" |dataTest$Make=="Mahindra"~ "Gama alta"
    )
  ) -> dataTest
```
...
for (i in 1:50){
  prediccion_knni= knn(data[,1:14], dataTest[,c(1:12,14,15)], k=i , cl=data$Gama)
...
Aquí podemos observar las precisiones que obtenemos en función de los vecinos que escogemos (en un intervalo de 1 a 50), pero podemos visualizarlo mucho mejor en el siguiente gráfico, donde marcamos el máximo en color rojo.

```{r knn 3}
#plot(precisionesknn,xlab="Número de vecinos",ylab="Precisión",type="l",col=3, main="Precisión en función de los vecinos elegidos")
#points(precisionesknn)
#points(which.max(precisionesknn),max(precisionesknn), col=2, pch=19,cex=1.5)
```


Resulta evidente que el número óptimo de vecinos que debemos escoger es `r which.max(precisionesknn)`, y como además es un número impar no nos genera ningún tipo de problema.

Por tanto, tomando un vecino, tenemos una precisión del `r round(precisionesknn[1]*100,2)`% para el conjunto de prueba y, evidentemente, una precisión del 100% para el conjunto de entrenamiento, ya que al escoger un vecino del conjunto de entrenamiento y comparar en ese conjunto, cada coche se comparará consigo mismo, y por tanto, se clasificará en su propia clase (es decir, con un vecino, cuando probamos con el conjunto de entrenamiento no hay lugar a error).


```{r, include=FALSE}
#pknn <- precisionesknn[1]
```
...
```{r bosques aleatorios 1}
# Ponemos una semilla para obtener siempre el mismo bosque (es un método aleatorio)
set.seed(16122020)

# PARÁMETROS DE LA FUNCIÓN
# ntrees = número de árboles del bosque
# mtry = número de variables de cada árbol
# replace = FALSE si queremos que las variables sean distintas o TRUE si se puediese tomar dos veces la misma variable
# importance = si queremos dar diferentes pesos a los árboles

# Probaremos con 10, 50, 100, 200, y 1000 árboles en el bosque
numArbolesRF <- c(10,50,100,200,500,1000)

# Aquí guardaremos el mejor modelo junto con el número de variables que tiene cada árbol, el número de árboles del bosque, la matriz de confusión asociada y la medida de precisión.
datosBosqueAleatorio <- vector("list",5)
library(randomForest)
for (v in data[,c(2:10,13,14)]) {                 # v será el número de variables de cada árbol
  for (a in numArbolesRF) {       # a será el número de árboles del bosque
    
    bosqueAleatorio <- randomForest(data[,c(2:10,13,14)], as.factor(data[,15]), ntree=a, importance=FALSE, proximity=TRUE, mtry=v, replace=FALSE)
    predRF <- predict(bosqueAleatorio, dataTest[,c(2:10,14,15)])
    tRF <- table(predRF, as.factor(dataTest[,13]))
    prRF <- medidaPrecision(tRF)
    
    if ((v==1 && a==numArbolesRF[1])){
      datosBosqueAleatorio[[1]] <- bosqueAleatorio
      datosBosqueAleatorio[[2]] <- v
      datosBosqueAleatorio[[3]] <- a
      datosBosqueAleatorio[[4]] <- tRF
      datosBosqueAleatorio[[5]] <- prRF
    }
    else if (as.numeric(prRF) > as.numeric(datosBosqueAleatorio[[5]])){
      datosBosqueAleatorio[[1]] <- bosqueAleatorio
      datosBosqueAleatorio[[2]] <- v
      datosBosqueAleatorio[[3]] <- a
      datosBosqueAleatorio[[4]] <- tRF
...
```{r bosques aleatorios 2}
predRF <- predict(datosBosqueAleatorio[[1]],dataTest[,c(2:10,14,15)])
tRF <- table(predRF,dataTest[,13])
pRF <- medidaPrecision(tRF)
```

Además, resultaría interesante comprobar cuáles son las variables más informativas de todas las variables explicativas disponibles, es decir, sobre qué características de los teoremas se hacen más preguntas en los árboles del bosque aleatorio conseguido.

```{r bosques aleatorios 3}
varImpPlot(datosBosqueAleatorio[[1]])
```


Así haciendolo facil... : 

```{r}
modelo <- randomForest(data$Gama~., data=data[,c(2:10,13,14)])
modelo
```
```{r}
modelo$importance
```
```{r}
predicciones <- predict(modelo, dataTest[,c(2:10,14,15)])
mc <- with(dataTest[,c(2:10,14,15)],table(predicciones, dataTest$Gama))
```

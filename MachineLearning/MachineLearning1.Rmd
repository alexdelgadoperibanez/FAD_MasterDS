...
tArb1 = table(predArbolInfo1, data[,15])
medidaPrecision(tArb1)
```

Obtenemos una precisión del %, lo que esperamos que disminuya al utilizar nuevos datos. Vamos a ver como clasifica nuestro árbol a nuevos teoremas. Para ello usaremos el conjunto de datos de testing y calcularemos la medida de precisión del resultado.
...
tArb2 = table(predArbolInfo2, dataTest[,13])
medidaPrecision(tArb2)
```

Efectivamente vemos como la precisión del % es menor, aunque tampoco ha disminuido mucho, lo que nos indica que es un buen árbol y que no hemos sobreajustado el modelo.
...
tArb3 = table(predArbolGini1, data[,15])
...
tArb4 = table(predArbolGini2,dataTest[,13])
pArb = medidaPrecision(tArb4)
pArb
```

Comprobamos que tanto con el conjunto de entrenamiento como con el conjunto de prueba las precisiones que obtenemos son idénticas para ambos métodos (información y Gini), así que la precisión que conseguimos utilizando árboles de decisión es de un %.
